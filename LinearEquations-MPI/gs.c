...
int processorWorkLoad = num / worldSize;

float *localNewX = malloc(sizeof(float *) * processorWorkLoad);
...
while (!done) {
    ...
    int localDone = 1;
    for (i = 0; i < processorWorkLoad; i++) {

        int localI = processorWorkLoad * myRank + i;
        localNewX[i] = b[localI];
        int j;
        for (j = 0; j < num; j++) {
            if (j != localI) {
                localNewX[i] -= a[localI][j]*x[j];
            }
        }
        localNewX[i] /= a[localI][localI];
        float errorMargin = (localNewX[i] - x[localI]) / localNewX[i];

        if (fabs(errorMargin) >= err) {
            localDone = 0;
        }
    }
